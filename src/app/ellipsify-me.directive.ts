...
export class EllipsifyMeDirective implements AfterViewInit {
  ...
  isTitleAttribute() {
    ...
    if (this.domElement.offsetWidth < this.domElement.scrollWidth){
      var overflowtext = this.domElement.offsetWidth - this.domElement.scrollWidth;
      this.renderer.addClass(this.domElement, 'is-ellipsis');
      this.renderer.setAttribute(this.domElement, 'title', this.domElement.textContent);
      this.renderer.setAttribute(this.domElement, 'style', '--text-indent:' + overflowtext + 'px');
    }
    else{
      this.renderer.removeAttribute(this.domElement, 'title');
      this.renderer.removeAttribute(this.domElement, 'style');
    }
